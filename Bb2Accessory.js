...
const { addQueue, queueExists } = require( "./Bb2PriorityPollingQueue" );
...
var { transposeConstantToValidValue,
    } = require( "./utils/transposeBB2Props" );

let isJSON = require( "./utils/isJSON" );
let isBb2Directive = require( "./utils/isBb2Directive" );
...
const Bb2Storage = require( "./utils/Bb2Storage" );
...
class Bb2Accessory
{
   constructor( log, config, api, STORED_DATA_ARRAY, parentInfo )
   {
      ...
      this.BB2 = ( parentInfo && parentInfo.BB2 ) ? parentInfo.BB2 : constants.STANDALONE;
      this.LEVEL = ( parentInfo && parentInfo.LEVEL !== undefined ) ? parentInfo.LEVEL + 1 : 0;
      this.createdBb2Accessories = ( parentInfo && parentInfo.createdBb2Accessories ) ? parentInfo.createdBb2Accessories : [ ];


      let typeMsg =  [ "", "Linked ", "Added " ][ this.LEVEL ] || "";

      if ( settings.bb2Dbg ) log.debug( chalk.blue ( `Creating ${ typeMsg }${ this.BB2 } Accessory type for : ${ config.displayName } LEVEL: ${ this.LEVEL }` ) );
      ...
      if ( existingData )
      {
         ...
         if ( settings.bb2Dbg ) this.log.debug(`Bb2Accessory: found existingData for ${ this.displayName }` );
         if ( existingData.storedValuesPerCharacteristic )
         {
            ...
            if ( settings.bb2Dbg ) this.log.debug( `Upgrading to bb2Storage` );
            this.bb2Storage = new Bb2Storage( this.log, existingData.storedValuesPerCharacteristic );
            this.STORED_DATA_ARRAY.push( { [ constants.UUID ]: uuid,
                                           [ constants.BB2_STORAGE_lv ]: this.bb2Storage
                                         }
                                        );
            //this.STORED_DATA_ARRAY.remove( existingData );
            removeFromArray( this.STORED_DATA_ARRAY, existingData );
         } else if ( existingData.bb2Storage )
         {
            //Z this.log.info( chalk.blue ( `THIS MSG TO BE REMOVED. Using existing bb2Storage for: ${ this.displayName }` ) );
            if ( settings.bb2Dbg ) this.log.debug( `Using existing bb2Storage` );
            this.bb2Storage = new Bb2Storage( this.log, existingData.bb2Storage );
            this.STORED_DATA_ARRAY.push( { [ constants.UUID ]: uuid,
                                           [ constants.BB2_STORAGE_lv ]: this.bb2Storage
                                         }
                                        );
            ...
            //Z log.info( chalk.blue ( `THIS MSG TO BE REMOVED. Unexpected empty bb2Storage for: ${ this.displayName }` ) );
            this.log.warn( `Unexpected empty bb2Storage` );
            this.bb2Storage = new Bb2Storage( this.log );
            this.STORED_DATA_ARRAY.push( { [ constants.UUID ]: uuid,
                                           [ constants.BB2_STORAGE_lv ]: this.bb2Storage
                                         }
                                        );
      ...
      {
         //Z log.info( chalk.blue ( `THIS MSG TO BE REMOVED. Creating new bb2Storage for: ${ this.displayName }` ) );
         if ( settings.bb2Dbg ) this.log.debug(`Bb2Accessory: creating new bb2Storage for ${ this.displayName }` );
      ...
      if ( ! this.stateChangeResponseTime )
         this.stateChangeResponseTime = BB2_DEVICE_TYPE_ENUM.properties[ this.typeIndex ].devicesStateChangeDefaultTime;
      ...
      // Convert the accessoriesConfig ( if any ) to an array of Bb2Accessory
      if ( this.accessoriesConfig && this.BB2 == constants.PLATFORM && this.LEVEL == 0 )
      {
         ...
         this.accessories = this.accessoryTypeConfigToBb2Accessories( this.accessoriesConfig, this );
         this.LEVEL = savedLevel;
      }

      // Convert the linkedTypes ( if any ) to an array of Bb2Accessory
      ...
      if ( this.linkedAccessoriesConfig && this.LEVEL == 0 )
      {
         ...
         this.linkedAccessories = this.accessoryTypeConfigToBb2Accessories( this.linkedAccessoriesConfig, this );
      }
      ...
      if ( this.LEVEL == 0 )
      {
         // if ( settings.bb2Dbg ) log.debug( "BB2=%s LEVEL=%s for %s", accessory.BB2, accessory.LEVEL, accessory.displayName );
         ...
         {
            if ( settings.bb2Dbg ) this.log.debug( `Setting up which characteristics will be polled for Linked Accessories of ${ this.displayName }` );
         ...
         {
            if ( settings.bb2Dbg ) this.log.debug( `Setting up which characteristics will be polled for Added Accessories of ${ this.displayName }` );
         ...
         if ( settings.bb2Dbg ) this.log.debug( `Setting up which characteristics will be polled for ${ this.displayName }` );
      ...
      if ( this.BB2 == constants.STANDALONE && this.LEVEL == 0 )
      {
         if ( settings.bb2Dbg ) log.debug( `Creating Standalone service for: ${ this.displayName }` );
         this.createServicesForStandaloneAccessoryAndItsChildren( this )
      }

   } // Bb2Accessory ( log, config, api, STORED_DATA_ARRAY, parentInfo )
   ...
   getServices( )
   {
      //if ( this.services )
      //{
      //   if ( settings.bb2Dbg ) this.log.debug( Fg.Red + "ZZZZ Returning:%s number of services for:%s" + Fg.Rm, this.services.length, this.displayName );
      //} else {
      //   if ( settings.bb2Dbg ) this.log.debug( Fg.Red + "ZZZZ Returning this.services:%s for:%s" + Fg.Rm, this.services, this.displayName );
      //}
   ...
   addRequiredCharacteristicStoredValues ( )
   {
      ...
      let properties = BB2_DEVICE_TYPE_ENUM.properties[ this.typeIndex ];

      // Check if required characteristics should be added, or TLV8 removed.
      for ( let accTypeEnumIndex = 0 ; accTypeEnumIndex < BB2_ACC_TYPE_ENUM.EOL; accTypeEnumIndex++ )
      {
         // Get the properties for this accessories device type
         let devProperties = BB2_DEVICE_TYPE_ENUM.properties[ this.typeIndex ];

         // See if the characteristic index is in the required characteristics of the device
         let requiredIndex = devProperties.requiredCharacteristics.Bb2indexOfEnum( i => i.type === accTypeEnumIndex );

         let format = BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props.format;

         // No matter what, remove it
         if ( format == this.api.hap.Formats.TLV8 && this.hV.allowTLV8 == false )
         {
            if ( this.bb2Storage.getStoredValueForIndex( accTypeEnumIndex ) != null )
            {
               this.bb2Storage.setStoredValueForIndex( accTypeEnumIndex, null );

               this.log.warn( `****** Removing TLV8 required characteristic: ${ BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].type }` );
            }
            continue;
         }

         // if it is required and not stored, add it
         if ( requiredIndex != -1 && this.bb2Storage.getStoredValueForIndex( accTypeEnumIndex ) == null )
         {
            this.log.warn( `**** Adding required characteristic ${ BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].type } for ${ this.displayName }` );
            ...
            if ( accTypeEnumIndex == BB2_ACC_TYPE_ENUM.ConfiguredName )
               defaultValue = getAccessoryName( this.config );

            if ( settings.bb2Dbg ) this.log.debug( `*****Adding default value ${ defaultValue } for: ${ this.displayName }` );

            this.bb2Storage.setStoredValueForIndex( accTypeEnumIndex, defaultValue );
         }
      }
   }
   ...
   checkPollingConfigForUnsetCharacteristics( pollingConfig )
   {
      ...
      if ( settings.bb2Dbg ) this.log.debug( `Checking ${ this.displayName } for polling of unset characteristics.` );
      ...
      pollingConfig.forEach( ( jsonPollingConfig ) =>
      {
         ...
         for ( key in jsonPollingConfig )
         {
            ...
            let rcDirective = isBb2Directive( key );
            if ( rcDirective == null )
            {
               rcDirective = isBb2Directive( key, true );
               if ( rcDirective != null )
               {
                  // warn now
                  this.log.warn( `The config.json Bb2 Polling Directive: ${ key } is Capitalized.  It should be: ${ rcDirective.key }. In the near future this will be an error for homebridge-ui integration.\nTo remove this Warning, Please fix your config.json.` );
            ...
            switch ( key )
            {
               ...
               case constants.CHARACTERISTIC:
               {
                  ...
                  if ( rcDirective.accTypeEnumIndex == null )
                  {
                     ...
                     this.log.warn( `The config.json Polling characteristic: ${ value } is Capitalized it should be: ${ rcDirective.type }.  In the near future this will be an Error so that Bb2 can use homebridge-ui.\nTo remove this Warning, Please fix your config.json.` );
                  }
                  accTypeEnumIndex = rcDirective.accTypeEnumIndex;

                  // We can do this as this is a new way to do things.
                  let storedValue = this.bb2Storage.getStoredValueForIndex( accTypeEnumIndex );
               ...
               default:
               {
                  ...
                  accTypeEnumIndex = BB2_ACC_TYPE_ENUM.Bb2indexOfEnum( key );
         ...
         if ( this.bb2Storage.getStoredValueForIndex( accTypeEnumIndex ) == undefined )
         {
         ...
         this.bb2Storage.setStoredValueForIndex( accTypeEnumIndex, valueToStore );
      });
   }

   createServicesForStandaloneAccessoryAndItsChildren( accessory )
   {
      if ( settings.bb2Dbg ) accessory.log.debug( chalk.blue( `createServicesFor${ this.BB2 }AccessoryAndItsChildren` ) );
      if ( accessory.ServiceCreated == true )
      {
         if ( settings.bb2Dbg ) accessory.log.debug( chalk.red( `SERVICES ALREADY CREATED FOR ${ this.displayName } ${ this.BB2 } ${ this.LEVEL }` ) );
         return;
      } else {
      ...
      let properties = BB2_DEVICE_TYPE_ENUM.properties[ accessory.typeIndex ];
      ...
      if ( settings.bb2Dbg ) accessory.log.debug( `Creating information service for standalone accessory: ${ accessory.displayName }` );
      ...
      if ( accessory.linkedAccessories )
      {
         ...
         {
            let properties = BB2_DEVICE_TYPE_ENUM.properties[ linkedAccessory.typeIndex ];

            // Standalone Step 4.
            //    const hdmi1InputService = this.tvAccessory.addService( this.Service.InputSource, `hdmi1', 'HDMI 1' );
            if ( settings.bb2Dbg ) accessory.log.debug( `Standalone Step 4. linkedAccessory( ${ accessory.displayName } ).service = new Service( ${ linkedAccessory.name }, ${ linkedAccessory.subType } )` );
            ...
            //if ( settings.bb2Dbg ) linkedAccessory.log.debug( "Creating information service for linkedAccessory:%s", linkedAccessory.displayName );
            //createAccessorysInformationService( linkedAccessory );

            if ( settings.bb2Dbg ) accessory.log.debug( `Standalone Step 5. ${ accessory.displayName }.service.addLinkedService( ${ linkedAccessory.displayName }.service` );
            // Standalone Step 5.
   ...
   setCachedValue( accTypeEnumIndex, characteristicString, value, callback )
   {
      ...
      if ( self.hV.statusMsg == "TRUE" )
         ...
         if ( settings.bb2Dbg ) self.log.debug( `setCachedvalue accTypeEnumIndex:( ${ accTypeEnumIndex } )-"${ characteristicString }" function for: ${ self.displayName } value: ${ value }` );


      // Save the cached value.
      // Fakegato does not need to be updated as that is done on a "Get".
      self.bb2Storage.setStoredValueForIndex( accTypeEnumIndex, value );
      ...
      if ( relatedCurrentAccTypeEnumIndex != null )
      {
         ...
         {
            let relatedCharacteristicString = BB2_ACC_TYPE_ENUM.properties[ relatedCurrentAccTypeEnumIndex ].type;
            self.log.info( chalk.blue( `Also Setting (Cached) ${ self.displayName } ${ relatedCharacteristicString }` ) + ` ${ value }` );
            self.bb2Storage.setStoredValueForIndex( relatedCurrentAccTypeEnumIndex, value );
         }
      }
   ...
   getCachedValue( accTypeEnumIndex, characteristicString, callback )
   {
      ...
      let storedValue = self.bb2Storage.getStoredValueForIndex( accTypeEnumIndex );
      ...
      if ( settings.bb2Dbg ) self.log.debug( `getCachedValue ${ characteristicString } for: ${ self.displayName } returned (CACHED) value: ${ storedValue }` );
   ...
   configHasCharacteristicProps( accTypeEnumIndex )
   {
      ...
      let characteristicProps = BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props;
      let type = BB2_ACC_TYPE_ENUM.accEnumIndexToLC( accTypeEnumIndex );
      let ucType = BB2_ACC_TYPE_ENUM.accEnumIndexToUC( accTypeEnumIndex );
   ...
   checkCharacteristicNeedsFixing( accessory, accTypeEnumIndex )
   {
      ...
      // We fix this here.
      if ( accTypeEnumIndex == BB2_ACC_TYPE_ENUM.CurrentHeatingCoolingState )
      {
         if ( settings.bb2Dbg ) this.log.debug( "fixing heatingCoolingState" );
         accessory.service.getCharacteristic(
            BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].
   ...
   {
       if ( settings.bb2Dbg ) accessory.log.debug( `Adding All Service Characteristics for: ${ accessory.displayName }` );
       ...
       for ( let accTypeEnumIndex = 0; accTypeEnumIndex < BB2_ACC_TYPE_ENUM.EOL; accTypeEnumIndex++ )
       {
          ...
          let uCCharacteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToUC( accTypeEnumIndex );

          // If there is a stored value for this characteristic ( defined by the config file )
          // Then we need to add the characteristic too
          let storedValue = accessory.bb2Storage.getStoredValueForIndex( accTypeEnumIndex );
          if ( storedValue != undefined )
          {
             if ( settings.bb2Dbg ) accessory.log.debug( "Found characteristic:%s value:%s for:%s",
             ...
             if ( ! accessory.service.testCharacteristic(
                  BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].characteristic ) )
             {
                //if ( settings.bb2Dbg ) accessory.log.debug( "Adding optional characteristic:%s for: %s", BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].type, this.displayName );
                if ( settings.bb2Dbg ) accessory.log.debug( "Adding optional characteristic:%s for: %s", uCCharacteristicString, this.displayName );

                accessory.service.addCharacteristic( BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].characteristic );
             }
             ...
             if ( props )
             {
                if ( settings.bb2Dbg ) accessory.log.debug( "Overriding characteristic %s props for: %s ", BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].type, this.displayName );
                  accessory.service.getCharacteristic( BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].
                         characteristic )
                  .setProps(
             ...
             // perms = BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props.perms
                  ...
                  perms.indexOf( this.api.hap.Perms.PAIRED_WRITE ) == -1 )
             {
                accessory.service.setCharacteristic(
                   BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].characteristic,
                      this.bb2Storage.getStoredValueForIndex( accTypeEnumIndex ) );
             }
                    ...
                    .characteristic ).listeners( "get" ).length == 0 )
             {
                     ...
                     perms.indexOf( this.api.hap.Perms.PAIRED_READ ) != -1 )
                {
                   ...
                   {
                      if ( settings.bb2Dbg ) this.log.debug( chalk.yellow( `Adding getCachedValue for ${ accessory.displayName } characteristic: ${ uCCharacteristicString } ` ) );
                      //Get cachedValue
                      accessory.service.getCharacteristic(
                         BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ]
                         .characteristic )
                  ...
                  {
                      if ( settings.bb2Dbg ) this.log.debug( chalk.yellow( `Adding priorityGetValue for ${ accessory.displayName } characteristic: ${ uCCharacteristicString }` ) );
                  ...
                  .characteristic ).listeners( "set" ).length == 0 )
             {
                     ...
                     perms.indexOf( this.api.hap.Perms.PAIRED_WRITE ) != -1 )
                {
                   ...
                   if ( ! accessory.polling ||
                        accessory.listOfPollingCharacteristics[ accTypeEnumIndex ] == undefined)
                   {
                      if ( settings.bb2Dbg ) this.log.debug( chalk.yellow( `Adding setCachedValue for ${ accessory.displayName } characteristic: ${ uCCharacteristicString } ` ) );
                      ...
                      if ( settings.bb2Dbg ) this.log.debug( chalk.yellow( `Adding prioritySetValue for ${ accessory.displayName } characteristic: ${ uCCharacteristicString } ` ) );
   ...
   updateAccessoryAttribute( accTypeEnumIndex, value )
   {
      ...
      this.bb2Storage.setStoredValueForIndex( accTypeEnumIndex, value );
      ...
      if ( this.loggingService )
      {
         ...
         switch ( this.eve )
         {
            case constants.FAKEGATO_TYPE_ENERGY:
            {
               ...
               firstParmValue = ( this.bb2Storage.testStoredValueForIndex( firstParmDirective.accTypeEnumIndex ) == undefined ) ?
                      firstParmValue : this.bb2Storage.getStoredValueForIndex( firstParmDirective.accTypeEnumIndex );

               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.POWER }: ${ firstParmValue }` );
            ...
            case constants.FAKEGATO_TYPE_ROOM:
            {
               ...
               firstParmValue = ( this.bb2Storage.testStoredValueForIndex( firstParmDirective.accTypeEnumIndex ) == undefined ) ?
                      firstParmValue : this.bb2Storage.getStoredValueForIndex( firstParmDirective.accTypeEnumIndex );
               secondParmValue = ( this.bb2Storage.testStoredValueForIndex( secondParmDirective.accTypeEnumIndex ) == undefined ) ?
                      secondParmValue : this.bb2Storage.getStoredValueForIndex( secondParmDirective.accTypeEnumIndex );
               thirdParmValue = ( this.bb2Storage.testStoredValueForIndex( thirdParmDirective.accTypeEnumIndex ) == undefined ) ?
                      thirdParmValue : this.bb2Storage.getStoredValueForIndex( thirdParmDirective.accTypeEnumIndex );


               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.TEMP }:${ firstParmValue } ${constants.HUMIDITY }:${ secondParmValue } ${ constants.PPM }:${ thirdParmValue }` );
            ...
            case constants.FAKEGATO_TYPE_WEATHER:
            {
               ...
               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.TEMP }: ${ firstParmValue } ${ constants.PRESSURE }: ${ secondParmValue } ${ constants.HUMIDITY }: ${ thirdParmValue }` );
            ...
            case constants.FAKEGATO_TYPE_DOOR:
            {
               ...
               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.STATUS } status: ${ firstParmValue }` );
            ...
            case constants.FAKEGATO_TYPE_MOTION:
            {
               ...
               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.STATUS }: ${ firstParmValue }` );
            ...
            case constants.FAKEGATO_TYPE_THERMO:
            {
               ...
               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.CURRENTTEMP }: ${ firstParmValue } ${ constants.SETTEMP }:${ secondParmValue } ${constants.VALVEPOSITION }:${ thirdParmValue } ` );
            ...
            case constants.FAKEGATO_TYPE_AQUA:
            {
               ...
               if ( settings.bb2Dbg ) this.log.debug( `Logging ${ constants.STATUS }: ${ firstParmValue } ${ constants.WATERAMOUNT }: ${ secondParmValue }` );
   ...
   setupAccessoryFakeGatoService( fakegatoConfig )
   {
      ...
      for ( let key in fakegatoConfig )
      {
         ...
         let rcDirective = isBb2Directive( key, false );
         if ( rcDirective == null )
         {
            rcDirective = isBb2Directive( key, true );
            if ( rcDirective != null )
            ...
            case constants.EVE:
               ...
               switch( value )
               {
                   ...
                   case constants.FAKEGATO_TYPE_AQUA:
                      break;
                   default:
                      throw new Error( `Invalid fakegato eve type: "${ value }". It must be one of ( ${ constants.FAKEGATO_TYPE_ENERGY }, ${ constants.FAKEGATO_TYPE_ROOM }, ${ constants.FAKEGATO_TYPE_WEATHER }, ${ constants.FAKEGATO_TYPE_DOOR }, ${ constants.FAKEGATO_TYPE_MOTION }, ${ constants.FAKEGATO_TYPE_THERMO }, ${ constants.FAKEGATO_TYPE_AQUA } ). Check the Bb2 README at: "https://github.com/simont77/fakegato-history".` );
                }
                break;
            ...
            case constants.VALVEPOSITION:
            {
               if ( value != "0" )
               {
                  ...
                  if ( rcDirective.accTypeEnumIndex == null )
                  {
                    ...
                    this.log.warn( `The config.json FakeGato characteristic: ${ value } is Capitalized it should be: ${ rcDirective.type }.  In the near future this will be an Error so that Bb2 can use homebridge-ui.\nTo remove this Warning, Please fix your config.json.` );
                  }
      ...
      if ( this.storage != undefined )
      {
         ...
         {
            this.log.warn( chalk.yellow( "WARNING" ) + `: Bb2 Unknown accessory config.storage:{ this.storage } Expected:${ constants.FS } or ${ constants.GOOGLEDRIVE } for: ${ this.displayName }` );
         }
      }
   ...
   parseKeyForCharacteristics( key, value, parseConfigShouldUseCharacteristicValues )
   {
      ...
      if ( BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].deprecated == true )
      {
      ...
      if ( Object.keys( BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].validValues ).length > 0 )
      {
         // Even if outputConsts is not set, just in case, transpose it anyway.
         value = transposeConstantToValidValue( BB2_ACC_TYPE_ENUM.properties, accTypeEnumIndex, value ) ;
      }


      // Return the appropriate type, by seeing what it is defined as in Homebridge,
      let properValue = BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].stringConversionFunction( value );
      if ( properValue == undefined )
      {
         // If the value is not convertable, just return it.
         this.log.warn( `parseKeyForCharacterisitcs: ${ this.displayName } ` + chalk.red( `Cannot convert value: ${ value } to ${ BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props.format } for ${ characteristicString }`  ) );
      ...
      this.bb2Storage.setStoredValueForIndex( accTypeEnumIndex, properValue );
   }
   ...
   processRequires( requiresArray )
   {
      ...
      for ( let argIndex = 0; argIndex < requiresArray.length; argIndex++ )
      {
         ...
         if ( settings.bb2Dbg ) this.log.debug( `Requiring ${ required }` );
   ...
   accessoryTypeConfigToBb2Accessories( config, parentInfo )
   {
      if ( ! config )
         return undefined;
      ...
      {
         let accessories = config.map( ( accessoryConfig ) => { return new Bb2Accessory( that.log, accessoryConfig, this.api, this.STORED_DATA_ARRAY, this ) } );

         // Put the accessories into their correct collection array.
         parentInfo.createdBb2Accessories.push( ...accessories );

         return accessories;
      }

      let accessory = new Bb2Accessory( that.log, config, this.api, this.STORED_DATA_ARRAY, this );

      // Put the accessory into its correct collection array.
      parentInfo.createdBb2Accessories.push( accessory );
   ...
   parseConfig( config, parseConfigShouldUseCharacteristicValues  )
   {
      ...
      for ( let key in config )
      {
         ...
         if ( rcDirective == null )
         {
            ...
            if ( rcDirective != null )
            {
               this.log.warn( `The config.json Bb2 Directive: ${ key } is Capitalized.  It should be: ${ rcDirective.key }. In the near future this will be an error for homebridge-ui integration.\nTo remove this Warning, Please fix your config.json.` );
               // create the proper lower case value
         ...
         switch ( key )
         {
            case constants.TYPE:
            {
               ...
               if ( rcValue.devEnumIndex == null )
               {
                  ...
                  // warn now
                  this.log.warn( `The config.json Bb2 device type: ${ value } is lowerCase.  It should be: ${ rcValue.deviceName }. In the near future this will be an error for homebridge-UI integration.\nTo remove this Warning, Please fix your config.json.` );

                  this.type = rcValue.deviceName;
               }
               this.typeIndex = rcValue.devEnumIndex;

               if ( BB2_DEVICE_TYPE_ENUM.properties[ this.typeIndex ].deprecated == true )
           ...
           case constants.ACCESSORY:
               ...
               // it easier for Bb2 to support homebridge-ui as well..
               throw new Error( `Accessory: has been deprecated by homebridge. Only Platform confgurations are supported by Bb2 >= 8.0 for: "${ this.displayName }".` );
           case constants.CATEGORY:
            ...
            case constants.CONSTANTS:
               this.log.warn( `Warning: ${ key } will soon been deprecated at the Accessory level. Please move it to where "Platform: "Bb2" is located in your config.json.` );
            ...
            case constants.LINKEDTYPES:
               if ( settings.bb2Dbg ) this.log.debug(  `parseConfig. Found linked Accessories` );
               this.linkedAccessoriesConfig = value;

               break;
            case constants.ACCESSORIES:
               if ( settings.bb2Dbg ) this.log.debug( `parseConfig. Found Accessories` );
            ...
            case constants.ALLOWTLV8:
               this.log.warn( `Warning: ${ key } will soon been deprecated at the Accessory level. Please move it to where "Platform: "Bb2" is located.` );
               this.log.warn( `This message will disappear when you have done so.` );
      ...
      if ( bb2Mode != null && this.polling )
      {
         if ( bb2Mode == "Demo" )
            throw new Error("Demo mode is achieved when there are no polling entries in your config.json");

         this.log.warn( `Bb2 has been simplified and optimized as per: https://git.io/JtMGR.` );
         this.log.warn( `To remove this message, just remove "Bb2_Mode" from your config.json` );
      }
      ...
      } else
      {
         ...
         if ( this.level != 1)
            this.log.info( chalk.blue( `Bb2 is running in Demo Mode for ${ this.displayName }` ) );
      }
      ...
      if ( this.typeIndex == BB2_DEVICE_TYPE_ENUM.Television )
      {
          if ( this.BB2 == constants.PLATFORM &&  ( ! this.publishExternally || ! this.category ) ||
               this.BB2 == constants.STANDALONE )
          {
          ...
          if ( this.BB2 == constants.PLATFORM && ! this.publishExternally && ( numberOfTVsPerBridge += 1 ) > 1 )
          {
      ...
      switch( pollingType )
      {
         case null:
         case undefined:
            if ( settings.bb2Dbg ) this.log.debug( `No polling configured.` );
            return;
         case Boolean:
            if ( settings.bb2Dbg ) this.log.debug( `Polling config is Default Polling. Nothing to check for unset polling characteristics` );
            return;
         case String:
      ...
      for ( let accTypeEnumIndex = 0; accTypeEnumIndex < BB2_ACC_TYPE_ENUM.EOL; accTypeEnumIndex ++ )
      {
         ...
         if ( storedValue != null )
         {
            ...
            let lcCharacteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToLC( accTypeEnumIndex );
            let ucCharacteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToUC( accTypeEnumIndex );
   ...
   getDevicesRelatedTargetAccTypeEnumIndex( accCurrentEnumIndex )
   {
      ...
      let requiredCharacteristicsArray = BB2_DEVICE_TYPE_ENUM.properties[ this.typeIndex ].requiredCharacteristics;
   ...
   determineCharacteristicsToPollForAccessory( accessory  )
   {
      ...
      {
         if ( settings.bb2Dbg ) this.log.debug( `Characteristic polling for: ${ accessory.displayName }` );
         accessory.polling.forEach( ( jsonPollingConfig ) =>
         {
               ...
               switch ( key )
               {
                  ...
                  case constants.CHARACTERISTIC:
                  {
                     ...
                     if ( rcDirective.accTypeEnumIndex == null )
                     {
                        ...
                        this.log.warn( `1. The config.json Polling characteristic: ${ value } is Capitalized it should be: ${ rcDirective.type }.  In the near future this will be an Error so that Bb2 can use homebridge-ui.\nTo remove this Warning, Please fix your config.json.` );
                     }
                     accTypeEnumIndex = rcDirective.accTypeEnumIndex;
            ...
            let characteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToUC( accTypeEnumIndex );

            if ( settings.bb2Dbg ) this.log.debug( `Setting up accessory: ${ accessory.displayName } for polling of: ${ characteristicString } timeout: ${ timeout } interval: ${ interval } queueName: "${ this.queue.queueName }"` );
      ...
      } else
      {
         ...
         BB2_DEVICE_TYPE_ENUM.properties[ accessory.typeIndex ].defaultPollingCharacteristics.forEach( defaultPollingAccTypeEnumIndex =>
         {

            // This has to be UC as it gets passed to the getValue cmd string
            let characteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToUC( defaultPollingAccTypeEnumIndex );
            ...
            if ( accessory.polling == true )
            {
               ...
               if ( settings.bb2Dbg ) this.log.debug( `Adding ${ record.accessory.displayName } ${ BB2_ACC_TYPE_ENUM.properties[ record.accTypeEnumIndex ].type }  record.timeout: ${ record.timeout } record.interval: ${ record.interval }  to Polled Queue ${ record.queueName }` );
      ...
      for( let accTypeEnumIndex in this.listOfPollingCharacteristics )
      {
         ...
         if ( relatedTargetAccTypeEnumIndex != null )
         {
               ...
               )
            {
               ...
               let characteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToLC( accTypeEnumIndex );
               let relatedCharacteristicString = BB2_ACC_TYPE_ENUM.accEnumIndexToLC( relatedTargetAccTypeEnumIndex );
...
function checkAccessoryForDuplicateUUID( accessory, uuid )
{
   ...
   if ( settings.bb2Dbg ) accessory.log.debug( `Checking ${ accessory.name } for Duplicate UUID: ${ accessory.uuid }` );

   for ( let existingAccessory in accessory.createdBb2Accessories )
   {
   ...
   if ( settings.bb2Dbg ) accessory.log.debug( `No Duplicate UUID's for this Accessory - ` + chalk.green( `OK` ) + `. Using: ${ accessory.uuid }` );
}

exports.Bb2Accessory = Bb2Accessory;
