...
class Bb2PriorityPollingQueue
{
   ...
   prioritySetValue( accTypeEnumIndex, characteristicString, timeout, stateChangeResponseTime,  value, homebridgeCallback )
   {
      // this is Accessory
      //
      //if ( settings.bb2Dbg ) this.log.debug(`prioritySetValue, asked to set: ${ characteristicString } to ${ value }`);
      ...
      {
         if ( settings.bb2Dbg ) this.log.debug(`prioritySetValue for ${ this.displayName }, homebridgeCallback returning error ${ this.errorValue } ${ this.errorString }`);
         ...
         this.service.getCharacteristic( BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].characteristic ).updateValue( storedValue );
      ...
      {
         if ( settings.bb2Dbg ) this.log.debug(`prioritySetValue for ${ this.displayName }, homebridgeCallback returning default success 0`);
   ...
   priorityGetValue( accTypeEnumIndex, characteristicString, timeout, homebridgeCallback )
   {
      ...
      // if ( settings.bb2Dbg ) this.log.debug(`priorityGetValue for ${ this.displayName }, asked to Get: ${ characteristicString }`);

      if ( this.errorValue != 0 )
      {
         // if ( settings.bb2Dbg ) this.log.debug(`priorityGetValue for ${ this.displayName }, homebridgeCallback returning error ${ this.errorValue } ${ this.errorString}`);
         homebridgeCallback( this.errorValue );
      } else
      {
         ...
         // if ( settings.bb2Dbg ) this.log.debug(`priorityGetValue for ${ this.displayName }, homebridgeCallback returning storedValue: ${ storedValue }`);
   ...
   {
      if ( settings.bb2Dbg ) this.log.debug( `Processing high priority queue "Set" entry: ${ entry.accTypeEnumIndex } length: ${ this.highPriorityQueue.length }` );
      ...
      this.qSetValue( entry.accessory, entry.accTypeEnumIndex, entry.characteristicString, entry.timeout, entry.value, function ( error )
      {
         ...
         if ( error == 0 )
         {
            ...
            entry.accessory.bb2Storage.setStoredValueForIndex( entry.accTypeEnumIndex, entry.value );
            ...
            {
            let relatedCurrentCharacteristicString = BB2_ACC_TYPE_ENUM.properties[ relatedCurrentAccTypeEnumIndex ].type;
         ...
         } else  // setValue failed
         {
            ...
            let storedValue = entry.accessory.bb2Storage.getStoredValueForIndex( entry.accTypeEnumIndex );
            entry.accessory.service.getCharacteristic( BB2_ACC_TYPE_ENUM.properties[ entry.accTypeEnumIndex ].characteristic ).updateValue( storedValue );
            ...
            if ( currentRetryCount >= queue.queueRetryCount )
            {
               ...
               // entry.accessory.service.getCharacteristic( BB2_ACC_TYPE_ENUM.properties[ entry.accTypeEnumIndex ].characteristic ).updateValue( entry.accessory.errorString );
   ...
   {
      if ( settings.bb2Dbg ) this.log.debug( `Processing high priority queue "Get" entry: ${ entry.accTypeEnumIndex } isUpdate: ${ entry.queueGetIsUpdate } length: ${ this.highPriorityQueue.length }` );
      ...
      this.qGetValue( entry.accessory, entry.accTypeEnumIndex, entry.characteristicString, entry.timeout, function ( error, properValue )
      {
         ...
         if ( error == 0 )
         {
            ...
            entry.accessory.bb2Storage.setStoredValueForIndex( entry.accTypeEnumIndex, properValue );

            // hmmm if ( entry.queueGetIsUpdate == true )
               entry.accessory.service.getCharacteristic( BB2_ACC_TYPE_ENUM.properties[ entry.accTypeEnumIndex ].characteristic ).updateValue( properValue );
   ...
   {
      if ( settings.bb2Dbg ) this.log.debug( `Processing low priority queue entry: ${ entry.accTypeEnumIndex }` );
      ...
      queue.qGetValue( entry.accessory, entry.accTypeEnumIndex, entry.characteristicString, entry.timeout, function ( error, properValue )
      {
         ...
         if ( error == 0 )
         {
            ...
            if ( settings.bb2Dbg ) entry.accessory.log.debug( `processEntryFromLowPriorityQueue calling updateValue properValue: ${ properValue }`);
   ...
   qGetValue( accessory, accTypeEnumIndex, characteristicString, timeout, callback )
   {
      ...
      if ( settings.bb2Dbg ) self.log.debug( `getValue: accTypeEnumIndex:( ${ accTypeEnumIndex } )-"${ characteristicString }" function for: ${ self.displayName } cmd: ${ cmd } timeout: ${ timeout }` );
      ...
      }).on('close', ( code ) =>
      {
         ...
         if ( words > 1 && BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props.allowedWordCount == 1 )
         {
         ...
         if ( settings.bb2Dbg ) self.log.debug( `getValue: ${ characteristicString } function for: ${ self.displayName } returned: ${ unQuotedReply }` );


         var transposed = transposeConstantToValidValue( BB2_ACC_TYPE_ENUM.properties, accTypeEnumIndex, unQuotedReply )
         if ( settings.bb2Dbg && transposed != unQuotedReply ) self.log.debug( `getValue: ${ characteristicString } for: ${ self.displayName } transposed: ${ transposed }` );


         // Return the appropriate type, by seeing what it is
         // defined as in Homebridge,
         let properValue = BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].stringConversionFunction( transposed );
         if ( properValue == undefined )
         {
            self.log.warn( `${ self.displayName } ` + chalk.red( `Cannot convert value: ${ unQuotedReply } to ${ BB2_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props.format } for ${ characteristicString }` ) );
         ...
         if ( settings.bb2Dbg && properValue != transposed ) self.log.debug( `getValue: ${ characteristicString } for: ${ self.displayName } properValue: ${ properValue }` );
   ...
   //
   //       - Where he value in <> is an one of BB2_ACC_TYPE_ENUM
   // ***********************************************
   qSetValue( accessory, accTypeEnumIndex, characteristicString, timeout, value, queueCallback )
   {
      ...
      {
         value = transposeValueToValidConstant( BB2_ACC_TYPE_ENUM.properties, accTypeEnumIndex, value );
      ...
      if ( settings.bb2Dbg ) self.log.debug( `setValue: accTypeEnumIndex:( ${ accTypeEnumIndex } )-"${ characteristicString }" function for: ${ self.displayName } ${ value }  cmd: ${ cmd } timeout: ${ timeout }` );
   ...
   processWormQueue( lastTransactionType, queue, lowPriorityEntry = null )
   {
      ...
      {
         // if ( settings.bb2Dbg ) queue.log.debug(`processWormQueue queue.inProgressSets > 0 : ${queue.inProgressSets}`);
      ...
      {
         // if ( settings.bb2Dbg ) queue.log.debug(`processWormQueue queue.lastGoodTransactionTime == 0`);
      ...
      if ( queue.highPriorityQueue.length > 0 )
      {
         ...
         if ( nextEntry.isSet == true )
         {
                 ...
                 nextEntry.accessory.queue.inProgressGets > 0 )
            {
               ...
               // if ( settings.bb2Dbg ) queue.log.debug(`processWormQueue queue.inProgressSets> 0 ${ nextEntry.accessory.queue.inProgressSets } ${ nextEntry.accessory.queue.inProgressGets }`);
           ...
           queue.queueStarted == true )
      {
         ...
         return true;
                 ...
                 queue.inProgressSets == 0 )
          {
             ...
             if ( settings.bb2Dbg ) this.log.debug( `Unhandled lastTransactionType: ${ lastTransactionType } inProgressSets: ${  queue.inProgressSets } inProgressGets: ${  queue.inProgressGets } queueStarted: ${ queue.queueStarted } lowQueueLen: ${ queue.lowPriorityQueue.length } hiQueueLen: ${ queue.highPriorityQueue.length }` );
   ...
   scheduleLowPriorityEntry( entry )
   {
      ...
      if ( settings.bb2Dbg ) accessory.log.debug( `Scheduling Poll of index: ${ entry.accTypeEnumIndex } characteristic: ${ entry.characteristicString } for: ${ accessory.displayName } timeout: ${ entry.timeout } interval: ${ entry.interval }` );
      ...
      queue.listOfRunningPolls[ accessory.displayName + entry.accTypeEnumIndex ] = setTimeout( ( ) =>
      {
         ...
         {
            if ( settings.bb2Dbg ) accessory.log.debug( `processsQueue returned false` );
   ...
   startQueue( queue, allDoneCallback )
   {
      ...
      if ( settings.bb2Dbg ) this.log.debug( `enablePolling for the first time` );
      ...
      } else
      {
         queue.lowPriorityQueue.forEach( ( entry, entryIndex ) =>
         {
            ...
            setTimeout( ( ) =>
            {
               if ( entryIndex == 0 && settings.bb2Dbg )
               {
               ...
               if ( settings.bb2Dbg ) entry.accessory.log.debug( `Kicking off polling for: ${ entry.accessory.displayName } ${ entry.characteristicString } interval:${ entry.interval }, staggered:${ staggeredDelays[ staggeredDelayIndex ] }` );
...
var addQueue = function( log, queueName, queueType = constants.DEFAULT_QUEUE_TYPE, queueRetryCount = constants.DEFAULT_WORM_QUEUE_RETRY_COUNT )
{
   ...
   queue = new Bb2PriorityPollingQueue( log, queueName, queueType, queueRetryCount );
...
var parseAddQueueTypes = function ( log, entrys )
{
   ...
   entrys.forEach( ( entry, entryIndex ) =>
   {
      ...
      if ( settings.bb2Dbg ) log.debug( `calling addQueue: ${ queueName } type: ${ queueType } retryCount: ${ queueRetryCount }` );
