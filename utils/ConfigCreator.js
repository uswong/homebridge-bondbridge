...
let bondbridgeConstants = {};
let bondbridgeQueueTypes = {};
...
function addFan(name, minStep, modelQueue, bondToken, device, IPA) {
  const fan = {
    type: "Fan",
    displayName: name,
    on: false,
...
function addLightbulbNoDimmer(name, accType, modelQueue, bondToken, device, IPA) {
  const lightbulb = {
    type: "Lightbulb",
    displayName: name,
    on: false,
...
function addLightbulbWithDimmer(name, accType, minStep, modelQueue, bondToken, device, IPA) {
  const lightbulb = {
    type: "Lightbulb",
    displayName: name,
    on: false,
...
function addTimerLightbulb(name, accType, deviceType, modelQueue, bondToken, timerDevice, device, IPA) {
...
async function main() {
  ...
  for (let n = 1; n <= noOfBondBridges; n++) {
    ...
    // Create config parts
    bondbridgeBasicKeys = createBasicKeys(debugBondBridge);

    if (CFsetupOption !== "doNotConfigure") {
      bondbridgeConstants = createConstants(IPA, ip, debug);
      bondbridgeQueueTypes = createQueueTypes(queue);
    }

    bondbridgeModelQueue = createModelQueue(model, bondid, queue);
      ...
      if (CFsetupOption !== "doNotConfigure") {
        if (/^[a-zA-Z0-9 ]*Fan$/.test(name)) {
          if (CFsetupOption !== "lightDimmer") {
            addFan(name, speedInterval, bondbridgeModelQueue, bondToken, device, IPA);
          }
          if (CFtimerSetup === "includeTimers") {
            addTimerLightbulb(`${name} Timer`, "fanTimer", "fanDevice", bondbridgeModelQueue, bondToken, timerDevice, device, IPA);
          }
        }
        if (/^[a-zA-Z0-9 ]*Light$/.test(name)) {
          if (CFsetupOption === "fanLight") {
            addLightbulbNoDimmer(name, "light", bondbridgeModelQueue, bondToken, device, IPA);
          } else if (CFsetupOption === "fanLightDimmer") {
            addLightbulbWithDimmer(name, "light", speedInterval, bondbridgeModelQueue, bondToken, device, IPA);
          } else if (CFsetupOption === "lightDimmer") {
            addLightbulbWithDimmer(`${name} Dimmer`, "dimmer", speedInterval, bondbridgeModelQueue, bondToken, device, IPA);
          }
          if (CFtimerSetup === "includeTimers" && CFsetupOption !== "fan") {
            addTimerLightbulb(`${name} Timer`, "lightTimer", "lightDevice", bondbridgeModelQueue, bondToken, timerDevice, device, IPA);
          }
        }
      }
    }
  ...
  console.log("DONE! BondBridge ConfigCreator completed successfully!");
